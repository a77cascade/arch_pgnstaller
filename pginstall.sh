...
#COMMON FUNCTIONS {{{
  ...
  sed -i "1,${LINES}d" ${mirror}

  # Uncomment Server List
  sed -i "s/#Server/Server/g" ${mirror}
...
#if [[ -f /home/ablive/Scripts/sharedfuncs ]]; then
#  source sharedfuncs
#else
...
# Desktop keymap
  sed -i "s/\gb/${XKEYMAP}/g" $MNT/home/ablive/.config/openbox/autostart
# Global setting
  sed -i "s/gb/${XKEYMAP}/g" $MNT/etc/X11/xorg.conf.d/01-keyboard-layout.conf
}
#}}}
...
install_archbang(){
  ...
  # set up kernel for mkiniticpio this needs to be tested...
  ...
  # Clean up new install scripts and configs
  rm -f ${MNT}/home/ablive/Scripts/{abinstall,zen-installer} &> /dev/null
# remove installers from menu and conky
  sed -i '/Installer/,+5d' ${MNT}/home/ablive/.config/openbox/menu.xml
  sed -i '/Install/d' ${MNT}/home/ablive/.config/conky/conky.conf
...
# clean out archiso files from install
  ...
  # remove pacman-init.service and etc-gpg thing
  rm -f ${MNT}/etc/systemd/system/pacman-init.service
  rm -f ${MNT}/etc/systemd/system/etc-pacman.d-gnupg.mount
  # path to desktop files
  path="${MNT}/usr/share/applications"

  # restore desktop files
  desk_file='volumeicon gparted conky tint2 tint2conf pcmanfm-desktop-pref' # qv4l2 qvidcap'

  for un in ${desk_file}
  do
     mv ${path}/${un}.hide ${path}/${un}.desktop
  done
...
configure_locale(){
  ...
  arch_chroot "sed -i '/en_GB\.UTF-8/ s/^#*/#/'  /etc/locale.gen"
...
configure_mkinitcpio(){
  ...
  # This needs testing to see if ucode gets placed in /boot
  proc=$(grep -m1 vendor_id /proc/cpuinfo | awk '{print $3}')
  if [ "${proc}" = "GenuineIntel" ]; then
    cp /run/archiso/bootmnt/arch/boot/intel-ucode.img ${MNT}/boot/
    cpu_id="intel"
  else
    cp /run/archiso/bootmnt/arch/boot/amd-ucode.img ${MNT}/boot/
    cpu_id="amd"
  fi
  # sed would be an easier option....
  mv "${MNT}/etc/mkinitcpio.d/linux.preset.ab" "${MNT}/etc/mkinitcpio.d/linux.preset"
  # delete whole hooks line 
  sed '/^HOOKS/d' "${MNT}/etc/mkinitcpio.conf"
  # then re-add HOOKS required..
  echo "HOOKS=(base udev memdisk block filesystems keyboard)" >> "${MNT}/etc/mkinitcpio.conf"
